...
public class AccountManager {
    ...
    private CallBackListener LoginCallBackLister = new CallBackListener<AccountBean>(){
        ...
        public void onSuccess(AccountBean loginInfo) {
            ...
            if (isSwitchAccount){
                CallBackToProject(TypeConfig.SWITCHACCOUNT, ErrorCode.SUCCESS,loginInfo, "user switchAccount success");
                isSwitchAccount = false; //置为false

            }else {
                CallBackToProject(TypeConfig.LOGIN,ErrorCode.SUCCESS,loginInfo, "user login success");
            }
        }
        ...
        public void onFailure(int code, String msg) {
            ...
            if (isSwitchAccount){
                ...
                if (code == ErrorCode.CANCEL){ //如果切换账号时,不走登录,给登出回调
                    CallBackToProject(TypeConfig.LOGOUT, ErrorCode.SUCCESS, null, "user logout success");
...
//                    @Override
//                    public void onClick(DialogInterface dialog, int index) {
//                        LoginCallBackLister.onFailure(ErrorCode.FAILURE,"login fail");
//                    }
//                });
//        builder.create().show();
//
//    }
    ...
    public void authLogin(Activity activity, HashMap<String,Object> loginMap){
        ...
        AccountBean loginInfo = new AccountBean();
        loginInfo.setLoginState(true); //将登录成功状态返回
        loginInfo.setUserToken(String.valueOf(loginMap.get("PARAMS_OAUTH_TOKEN")));
        loginInfo.setUserID(String.valueOf(loginMap.get("PARAMS_OAUTH_URL")));
        loginInfo.setUserName("用户名暂不返回"); //聚合将用名设置为UserID
        LoginCallBackLister.onSuccess(loginInfo);
    }
    ...
    public void switchAccount(Activity activity){
        ...
        CallBackToProject(TypeConfig.SWITCHACCOUNT, ErrorCode.SUCCESS, null, "user logout success");
    }
