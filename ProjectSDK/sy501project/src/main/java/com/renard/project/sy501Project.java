 ...
 * @author Lemon酱
 */
public class sy501Project extends Project {
    ...
    public void onConfigurationChanged(Activity activity, Configuration config) {
        ...
        if (InitManager.getInstance().getInitState()){
            super.onConfigurationChanged(activity, config);
            channel.onConfigurationChanged(activity,config);
        }
    }
}
